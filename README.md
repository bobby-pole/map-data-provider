...
2. Request cached layers for any of the nine required domains or the optional `telecom` domain (`power`, `emergency`, `public`, `transport`, `bridges`, `water`, `gas`, `sewer`, `industrial`, `telecom`) through read-only API endpoints or multi-domain export (`GET /api/aoi/:aoiId/export?domains=...`).
...
A compatible client requests any subset of the nine required domains (power, emergency, public, transport, bridges, water, gas, sewer, industrial) and the optional telecom domain for the Rybnik AOI.
...
- Catalogued AOI runtime profiles for the nine required domains plus optional `telecom`. Telecom separates explicitly tagged towers/masts, facilities and lines; missing lines remain a visible source gap, while KIUT telecom is reference-only. Existing Rybnik fixture packs remain explicit demo fallbacks; unavailable AOI data is reported as a source gap, never as empty analytical vectors.
...
`POST /api/aoi/requests` remains the legacy `rybnik_60km/power` compatibility path. `GET /api/aoi/catalog` returns the dated, source-labelled bounded PRG administrative selection catalogue. `POST /api/aoi/runtime-requests` accepts a `provider_aoi_request/v2` point/radius or administrative selection plus requested categories. It derives a deterministic request/cache identity, coalesces equivalent local requests and reuses a fresh local runtime result for 24 hours. On an AOI cache miss it acquires any of the nine required profiles plus the optional qualified `telecom` profile, validates and writes a local domain pack plus PMTiles presentation, then returns it as `ready`. Telecom admits only explicit telecommunications tags: GSM masts/towers, antenna/site facilities and `communication=line` or `cable=communication` routes; empty lines remain `needs_source`. KIUT/orthophoto contexts remain `reference_only` and are never vectorized.
