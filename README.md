...
- **Controlled demo acquisition**: Production uses `MDQ_RUNTIME_MODE=demo_fixed_aoi` and exposes `POST /api/aoi/demo-acquisitions/rybnik_gmina_demo` for a server-validated custom AOI. The public flow accepts a map point whose circle is at most 10 km and remains inside Poland, or a PRG selection contained in one county. It always prepares all 11 provider domains and builds a PMTiles presentation so the map can switch to the newly published AOI. Generic runtime endpoints, force refresh and out-of-policy requests return typed `demo_aoi_restricted` (HTTP 403).
