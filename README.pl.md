...
- `demo_fixed_aoi` — publiczne ustawienie VPS opisane poniżej. Eksponuje ograniczoną akwizycję sterowaną punktem/PRG; każde żądanie poza polityką lub wymuszenie odświeżenia jest odrzucane z typowaną odpowiedzią `demo_aoi_restricted` (HTTP 403).
...
- **Kontrolowana akwizycja demo**: Produkcja używa `MDQ_RUNTIME_MODE=demo_fixed_aoi` i eksponuje `POST /api/aoi/demo-acquisitions/rybnik_gmina_demo` dla zwalidowanego AOI z mapy. Demo przyjmuje punkt z promieniem do 10 km tylko gdy koło pozostaje w Polsce albo wybór PRG mieszczący się w jednym powiecie. Zawsze przygotowuje wszystkie 11 domen i buduje prezentację PMTiles, aby mapa mogła przełączyć się na nowo opublikowany AOI. Ogólne endpointy, force refresh i żądania poza limitem zwracają typowane `demo_aoi_restricted` (HTTP 403).
...
- **Granica runtime**: Publiczne demo może zademonstrować jedno koalescowane, cache-first ograniczone żądanie. Tryby `local_bounded` i uwierzytelniony `trusted` pozwalają na 30 km lub do trzech sąsiadujących powiatów i mogą budować PMTiles dla Steel Sentinel. VPS nie staje się otwartym proxy Overpass.
