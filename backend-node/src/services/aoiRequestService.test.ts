...
describe("AOI request orchestration", () => {
  ...
  it("runs worker for missing cache and returns refresh", async () => {
    ...
    const result = await requestAoi("rybnik_35km", "power", { readCachedLayers: async () => (++calls === 1 ? [] : [metadata("2026-07-22T00:00:00Z")]), runWorker: worker, now: () => new Date("2026-07-22T01:00:00Z") });
  ...
  it("runs worker for stale cache and propagates failure", async () => {
    await expect(requestAoi("rybnik_35km", "power", { readCachedLayers: async () => [metadata("2026-07-20T00:00:00Z")], runWorker: async () => { throw new Error("boom"); }, now: () => new Date("2026-07-22T01:00:00Z") })).rejects.toMatchObject({ kind: "worker_failed" });
  });
  it("has deterministic freshness and rejects unsupported combinations", async () => {
    ...
    await expect(requestAoi("rybnik_35km", "transport")).rejects.toMatchObject({ kind: "invalid_request" });
  });
});
