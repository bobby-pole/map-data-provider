...
const allowedTransitions: Record<
  IssueReviewRecord["status"] | "open",
  IssueReviewRecord["status"][]
> = {
...
export async function getReviewedIssues(
  aoiId: string,
  paths?: IssueStorePaths,
): Promise<ReviewedIssue[]> {
  validateAoiId(aoiId);
  const [snapshot, store] = await Promise.all([
    readIssueSnapshot(aoiId, paths),
    readReviewStore(paths),
  ]);
...
async function updateIssueReviewLocked(
  ...
  if (!issue) {
    throw new ProviderDataError(
      "not_found",
      `No generated issue '${issueId}' exists for AOI '${aoiId}'.`,
    );
  }
  ...
  if ((existing?.updated_at ?? null) !== update.expected_updated_at) {
    throw new ProviderDataError(
      "conflict",
      "The review was updated by another request. Reload the issue before saving.",
    );
  }
  if (!allowedTransitions[currentStatus].includes(update.status)) {
    throw new ProviderDataError(
      "invalid_request",
      `Review transition '${currentStatus}' -> '${update.status}' is not allowed.`,
    );
  }
  ...
  await writeReviewStore(
    {
      review_store_version: "provider_issue_reviews/v1",
      reviews: replaceReview(store.reviews, existing, record),
    },
    resolvedPaths,
  );
...
async function readIssueSnapshot(aoiId: string, paths?: IssueStorePaths) {
  const snapshot = generatedIssueSnapshotSchema.parse(
    await readJson(resolvePaths(paths).generatedIssuesPath, "generated issue snapshot"),
  );
  if (snapshot.aoi_id !== aoiId) {
    throw new ProviderDataError("not_found", `No generated issues exist for AOI '${aoiId}'.`);
  }
  return snapshot;
}

async function readReviewStore(paths?: IssueStorePaths) {
  return issueReviewStoreSchema.parse(
    await readJson(resolvePaths(paths).reviewsPath, "issue review store"),
  );
}
...
function mergeIssueReview(
  issue: GeneratedIssue,
  aoiId: string,
  reviews: IssueReviewRecord[],
): ReviewedIssue {
...
function replaceReview(
  records: IssueReviewRecord[],
  existing: IssueReviewRecord | undefined,
  next: IssueReviewRecord,
): IssueReviewRecord[] {
  return existing
...
function resolvePaths(paths?: IssueStorePaths): ResolvedIssueStorePaths {
  return {
    generatedIssuesPath: paths?.generatedIssuesPath ?? defaultIssueSnapshotPath,
    reviewsPath: paths?.reviewsPath ?? defaultReviewStorePath,
  };
}
...
function validateAoiId(aoiId: string): void {
  if (!/^[a-z0-9_]+$/.test(aoiId)) {
    throw new ProviderDataError(
      "invalid_request",
      "AOI must use lowercase letters, digits and underscores only.",
    );
  }
}
