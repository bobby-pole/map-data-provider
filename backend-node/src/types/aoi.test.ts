...
describe("provider_aoi/v1 contracts", () => {
  it("accepts bounded circle and approved administrative-reference request shapes", () => {
    expect(
      aoiInputSchema.parse({
        type: "circle",
        longitude: 18.546285,
        latitude: 50.102174,
        radius_m: 35_000,
      }),
    ).toMatchObject({ type: "circle" });
    expect(
      aoiInputSchema.parse({ type: "administrative_reference", reference_id: "prg_gmina_rybnik" }),
    ).toMatchObject({ type: "administrative_reference" });
  });

  it("rejects unsafe circles and uncontracted geometry fields", () => {
    expect(() =>
      aoiInputSchema.parse({ type: "circle", longitude: 18.5, latitude: 50.1, radius_m: 0 }),
    ).toThrow();
    expect(() =>
      aoiInputSchema.parse({
        type: "circle",
  ...
  it("accepts only explicit v2 runtime AOI modes and provider categories", () => {
    expect(
      providerRuntimeRequestSchema.parse({
        aoi: {
          type: "administrative_selection",
          unit_ids: ["county_rybnik_city", "county_rybnicki"],
        },
        profiles: ["power", "water"],
      }),
    ).toMatchObject({ profiles: ["power", "water"] });
    expect(() =>
      providerRuntimeRequestSchema.parse({
        aoi: { type: "point_radius", longitude: 18.5, latitude: 50.1, radius_m: 1_000 },
        profiles: ["unknown"],
      }),
    ).toThrow();
  });
        ...
        contexts: [
          {
            ...
            output_kind: "reference_descriptor",
            status: "reference_only",
            detail: "Rendered reference.",
          },
        ],
      }),
  ...
  it("keeps unavailable circuit enrichment explicit so a targeted recovery can be attempted", () => {
    const unavailableEvidence = powerCircuitEvidencePayloadSchema.parse({
      ...
      snapshot_at: "2026-08-14T15:53:33.572Z",
      bbox: [18.4, 50, 18.6, 50.2],
      source_checksum: null,
      relations: [],
      reverse_member_index: {},
      availability: "unavailable",
      limitations: ["The initial bounded relation request was rate limited."],
    });
    expect("availability" in unavailableEvidence && unavailableEvidence.availability).toBe(
      "unavailable",
    );
    expect(
      mapCircuitListResponseSchema.parse({
        ...
        state: "unavailable",
        circuits: [],
        limitations: ["Targeted recovery was unavailable."],
      }).state,
    ).toBe("unavailable");
  });
});
