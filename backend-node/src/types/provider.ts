...
export const aoiCircleInputSchema = z
  .object({
...
export const administrativeAoiReferenceInputSchema = z
  .object({
...
export const aoiInputSchema = z.union([
  aoiCircleInputSchema,
  administrativeAoiReferenceInputSchema,
]);

const runtimePointRadiusAoiSchema = z
  .object({
    type: z.literal("point_radius"),
    longitude: z.number().finite(),
    latitude: z.number().finite(),
    radius_m: z.number().finite(),
  })
  .strict();
const runtimeAdministrativeAoiSchema = z
  .object({
    type: z.literal("administrative_selection"),
    unit_ids: z.array(providerIdentifierSchema).min(1),
  })
  .strict();
export const runtimeAoiInputSchema = z.union([
  runtimePointRadiusAoiSchema,
  runtimeAdministrativeAoiSchema,
]);
...
export const providerRuntimeRequestSchema = z
  .object({
...
const runtimeGeometrySchema = z
  ...
  .strict();
const runtimeResolvedAoiSchema = z
  .object({
    ...
    constraints: z.record(z.string(), z.number().positive()),
    aliases: z.array(providerIdentifierSchema),
  })
  .strict();
export const providerRuntimeResponseSchema = z
  .object({
    status: z.literal("ok"),
    request_contract_version: z.literal("provider_aoi_request/v2"),
    request_id: providerIdentifierSchema,
    cache_key: providerIdentifierSchema,
    aoi: runtimeResolvedAoiSchema,
    pipeline_version: z.string().min(1),
    job_state: z.literal("ready"),
    request_result: z.enum(["cache", "refresh"]),
    cached_at: z.string().datetime(),
    profiles: z.array(
      z
        .object({
          domain: runtimeProfileSchema,
          source_registry_id: z.string().min(1),
          source_role: z.enum(["analytical", "reference", "review"]),
          output_kind: z.enum(["analytical_vector", "reference_descriptor", "derived_context"]),
          query_version: z.string().min(1),
    ...
    outcomes: z.array(
      z
        .object({
          ...
          failure_reason: z.enum(["timeout", "acquisition_error"]).nullable(),
          artifact_aoi_id: providerIdentifierSchema.nullable(),
          cache_status: z.enum(["fresh", "missing"]),
    ...
    contexts: z.array(
      z
        .object({
          ...
          status: z.enum(["ready", "needs_source", "reference_only", "pending_qualification"]),
          detail: z.string().min(1),
        })
        .strict(),
    ),
  ...
  .strict();
export const providerRuntimeJobSchema = z
  .object({
  ...
  .strict();
export const administrativeCatalogResponseSchema = z
  .object({
    catalog_version: z.literal("prg_administrative_catalog/v2"),
    source_registry_id: z.literal("prg_wfs"),
    snapshot_at: z.string().datetime(),
    source_crs: z.literal("EPSG:4326"),
    ...
    units: z.array(
      z
        .object({
          ...
          kind: z.enum(["voivodeship", "county", "gmina"]),
          name: z.string().min(1),
          prg_id: z.string().min(1),
  ...
  .strict();
export const administrativeBoundaryRequestSchema = z
  .object({ unit_ids: z.array(providerIdentifierSchema).min(1) })
  .strict();
export const administrativeBoundaryResponseSchema = z
  .object({
    response_version: z.literal("provider_administrative_boundary/v1"),
    aoi: runtimeResolvedAoiSchema,
    metric_area_sq_m: z.number().nonnegative(),
    within_provider_area_limit: z.boolean(),
    message: z.string().min(1),
  })
  .strict();
export const providerRuntimePreflightResponseSchema = z
  .object({
    response_version: z.literal("provider_aoi_preflight/v1"),
    status: z.enum(["ready", "blocked"]),
    code: z.enum(["bounded_provider_request", "aoi_area_limit"]),
    message: z.string().min(1),
    aoi: runtimeResolvedAoiSchema,
    metric_area_sq_m: z.number().nonnegative(),
  })
...
export const reviewedIssueSchema = generatedIssueSchema
  .extend({ review: issueReviewSchema })
  .strict();
...
export const sourceQualificationSchema = z.enum([
  "qualified_free",
  "pending_qualification",
  "rejected",
]);
export const sourceDistributionSchema = z
  .object({
...
export const sourceRegistryV2EntrySchema = z
  .object({
    id: z.string().min(1),
    name: z.string().min(1),
    ...
    limitations: z.array(z.string()),
    cache_provenance: z
      .object({ required_fields: z.array(z.string().min(1)).min(1) })
      .strict()
      .optional(),
  })
  .strict()
  .superRefine((source, context) => {
    ...
    if (
      ["paid", "agreement_only", "private_partner"].includes(source.access_method) &&
      source.qualification === "qualified_free"
    ) {
      context.addIssue({
        code: "custom",
        message: "Restricted access source cannot be qualified free.",
      });
    }
    ...
    if (source.usage_role !== "analytical" && source.eligible_for_analysis) {
      context.addIssue({
        code: "custom",
        message: "Only analytical sources can be eligible for analysis.",
      });
    }
    if (source.usage_role !== "analytical" && source.distribution.public_export !== "prohibited") {
      context.addIssue({
        code: "custom",
        message: "Reference or review sources cannot enter public analytical export.",
      });
    }
    if (
      source.qualification !== "qualified_free" &&
      source.distribution.public_export !== "prohibited"
    ) {
      context.addIssue({
        code: "custom",
        message: "Unqualified sources cannot enter public export.",
      });
    }
    if (source.distribution.public_export === "allowed" && source.not_authoritative) {
      context.addIssue({
        code: "custom",
        message: "Non-authoritative sources cannot enter public analytical export.",
      });
    }
    if (
      source.usage_role === "analytical" &&
      source.data_kind === "vector" &&
      source.qualification === "qualified_free" &&
      !source.cache_provenance
    ) {
      context.addIssue({
        code: "custom",
        message: "Analytical vector sources require cache provenance.",
      });
    }
  });
  ...
  .superRefine((registry, context) => {
    ...
    for (const requiredId of [
      ...
      if (!sourceIds.includes(requiredId)) {
        context.addIssue({
          code: "custom",
          message: `Source registry v2 is missing required source family: ${requiredId}.`,
        });
      }
    }
  });

export function isPublicExportEligible(
  source: z.infer<typeof sourceRegistryV2EntrySchema>,
): boolean {
  return (
    source.distribution.public_export === "allowed" &&
    source.qualification === "qualified_free" &&
    source.usage_role === "analytical" &&
...
export function validateOrderedSourceProvenance(
  ...
  if (provenance.length === 0) {
    throw new Error("Ordered provenance must contain at least one source.");
  }
  const sourceIds = new Set<string>();
  for (const record of provenance) {
    const parsed = sourceProvenanceSchema.parse(record);
    if (sourceIds.has(parsed.source_id)) {
      throw new Error("Ordered provenance source IDs must be unique.");
    }
    const source = registry.sources.find((candidate) => candidate.id === parsed.source_id);
    if (!source) {
      throw new Error(`Unknown source registry ID: ${parsed.source_id}.`);
    }
    if (publicExport && !isPublicExportEligible(source)) {
      throw new Error(`Source ${parsed.source_id} is not eligible for public export.`);
    }
    sourceIds.add(parsed.source_id);
  }
}

export const domainPackArtifactSchema = z
  .object({
    id: z.string().min(1),
    kind: z.enum([
      "native_vector",
...
export const domainPackV2Schema = z
  .object({
  ...
  .superRefine((pack, context) => {
    ...
    if (new Set(ids).size !== ids.length) {
      context.addIssue({ code: "custom", message: "Domain-pack artifact IDs must be unique." });
    }
...
export const sourceAvailabilityEntrySchema = z
  .object({
    source_id: z.string().min(1),
    availability: z.enum(["available", "unavailable", "not_eligible", "reference_only"]),
    aoi_coverage: z.enum(["covered", "uncovered", "not_applicable"]),
    feature_state: z.enum(["available", "empty", "not_applicable"]),
    evidence_timestamp: z.string().datetime(),
    fresh_after_days: z.number().int().positive(),
    evidence: z.string().min(1),
    freshness: z.enum(["fresh", "stale"]),
    eligibility: z.enum(["allowed", "rejected", "not_comparable"]),
    actionable_gap: z.boolean(),
  })
  .strict();
export const sourceAvailabilityReportSchema = z
  .object({
    report_version: z.literal("provider_source_availability/v1"),
    aoi_id: providerIdentifierSchema,
    evidence_timestamp: z.string().datetime(),
    sources: z.array(sourceAvailabilityEntrySchema).min(1),
  })
  .strict();
...
const presentationBoundsSchema = z
  .tuple([
...
export const mapPresentationLayerSchema = z
  .object({
...
export const mapPresentationManifestSchema = z
  .object({
    presentation_version: z.literal("provider_map_presentation/v1"),
    aoi_id: providerIdentifierSchema,
    domain: providerIdentifierSchema,
    parent_domain_pack: z
      .object({
    ...
    archive: z
      ...
      .strict()
      .refine((archive) => archive.min_zoom <= archive.max_zoom, {
        message: "Presentation zoom range is invalid.",
      }),
    ...
    benchmark: z
      .object({
        ...
        baseline: z
          .object({
            delivery: z.literal("full_geojson_to_leaflet"),
            feature_count: z.number().int().nonnegative(),
        ...
        presentation: z
          .object({
            delivery: z.literal("pmtiles_mvt_range_reads"),
...
export const mapPresentationResponseSchema = mapPresentationManifestSchema
  .extend({
...
export const mapPresentationListResponseSchema = z
  .object({
...
export const mapFeatureDetailResponseSchema = z
  .object({
...
const circuitMemberSchema = z
  .object({
    source_id: osmElementIdentifierSchema,
    role: z.string(),
    availability: z.string().optional(),
    endpoint_evidence: z
      .object({ start: osmElementIdentifierSchema, end: osmElementIdentifierSchema })
      .strict()
      .optional(),
    geometry: z
      .object({
...
const powerCircuitSchema = z
  ...
  .strict();
const powerCircuitSummarySchema = z
  .object({
    relation_id: z.string().regex(/^relation\/\d+$/),
    tags: z.record(z.string(), z.string()),
    aoi_coverage: z.literal("bounded_source_snapshot"),
  ...
  .strict();
const availablePowerCircuitEvidencePayloadSchema = z
  .object({
  ...
  .strict();
const unavailablePowerCircuitEvidencePayloadSchema = z
  .object({
    relation_evidence_version: z.literal("osm_power_relation_evidence/v2"),
    source: z.literal("OpenStreetMap"),
    snapshot_at: z.string().datetime(),
    ...
    reverse_member_index: z
      .record(z.string(), z.array(z.string().regex(/^relation\/\d+$/)))
      .refine((index) => Object.keys(index).length === 0),
    availability: z.literal("unavailable"),
    limitations: z.array(z.string().min(1)).min(1),
  })
  .strict();
export const powerCircuitEvidencePayloadSchema = z.union([
  availablePowerCircuitEvidencePayloadSchema,
  unavailablePowerCircuitEvidencePayloadSchema,
]);
export const mapCircuitListResponseSchema = z
  .object({
  ...
  .strict();
export const mapCircuitDetailResponseSchema = z
  .object({
