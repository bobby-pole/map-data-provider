...
describe("source_registry/v2 contract", () => {
  it("accepts the shared valid fixture and preserves source-family dimensions", async () => {
    ...
    expect(sources.get("openstreetmap")).toMatchObject({ data_kind: "vector", format: "osm_query", authority: "community", usage_role: "analytical" });
  ...
  it.each(["invalid-incomplete-v2.json", "invalid-contradictory-v2.json"])("rejects shared invalid fixture %s", async (name) => {
  ...
  it("enforces ordered provenance and public-export eligibility", async () => {
    ...
    expect(() => validateOrderedSourceProvenance([{ source_id: "kiut_gesut_wms", contribution_role: "validation_reference" }], registry, true)).toThrow(/not eligible/);
    expect(() => validateOrderedSourceProvenance([
