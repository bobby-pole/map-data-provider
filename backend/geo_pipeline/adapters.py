...
def _district_heating_live() -> None:
    raise AdapterError(
        "Use the AOI runtime path for bounded district-heating OSM acquisition; "
        "the fixture adapter is offline-only."
    )
...
DISTRICT_HEATING_ADAPTER = DomainAdapter(
    ...
    run_live=_district_heating_live,
    build_domain_pack=lambda root: build_rybnik_district_heating_domain_pack(root=root),
)
