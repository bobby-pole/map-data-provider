...
CATALOG_PATH = (
    ...
    / "fixtures"
    / "aoi"
    / "prg_administrative_catalog.geojson"
)
POLAND_BOUNDS = (14.05, 49.0, 24.25, 55.0)
ProfileOutcome = Literal[
    "ready", "needs_source", "reference_only", "pending_qualification", "failed"
]
...
PROFILES: tuple[ProviderProfile, ...] = (
    ProviderProfile(
        "power",
        POWER_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        POWER_OSM_QUERY.query_version,
        POWER_OSM_QUERY.tags,
        True,
    ),
    ProviderProfile(
        "emergency",
    ...
    ProviderProfile(
        "transport",
        TRANSPORT_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        TRANSPORT_OSM_QUERY.query_version,
        TRANSPORT_OSM_QUERY.tags,
        True,
    ),
    ProviderProfile(
        "bridges",
    ...
    ProviderProfile(
        "water",
        WATER_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        WATER_OSM_QUERY.query_version,
        WATER_OSM_QUERY.tags,
        True,
    ),
    ProviderProfile(
        "gas",
        GAS_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        GAS_OSM_QUERY.query_version,
        GAS_OSM_QUERY.tags,
        True,
    ),
    ProviderProfile(
        "sewer",
        SEWER_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        SEWER_OSM_QUERY.query_version,
        SEWER_OSM_QUERY.tags,
        True,
    ),
    ProviderProfile(
        "industrial",
        INDUSTRIAL_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        INDUSTRIAL_OSM_QUERY.query_version,
        INDUSTRIAL_OSM_QUERY.tags,
        True,
    ),
    ProviderProfile(
        "telecom",
        TELECOM_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        TELECOM_OSM_QUERY.query_version,
        TELECOM_OSM_QUERY.tags,
        True,
    ),
    ProviderProfile(
        "district_heating",
        DISTRICT_HEATING_OSM_QUERY.source_registry_id,
        "analytical",
        "analytical_vector",
        DISTRICT_HEATING_OSM_QUERY.query_version,
        DISTRICT_HEATING_OSM_QUERY.tags,
        True,
    ),
)
...
def administrative_boundary(unit_ids: list[Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "message": "Selected PRG boundary is within the current provider area limit."
        if area_sq_m <= MAX_AREA_SQ_M
        else "Selected PRG boundary exceeds the current provider area limit; it can be viewed but cannot start OSM acquisition.",
    }
...
def preflight_runtime_request(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(aoi_value, dict) and aoi_value.get("type") == "administrative_selection":
        if (
            set(aoi_value) != {"type", "unit_ids"}
            or not isinstance(aoi_value["unit_ids"], list)
            or not aoi_value["unit_ids"]
        ):
        ...
        if area_sq_m > MAX_AREA_SQ_M:
            return {
                "response_version": "provider_aoi_preflight/v1",
                "status": "blocked",
                "code": "aoi_area_limit",
...
def resolve_runtime_request(value: dict[str, Any]) -> dict[str, Any]:
    ...
    identity = {
        ...
        "profiles": [
            {"domain": profile.domain, "query_version": profile.query_version}
            for profile in profiles
        ],
        "pipeline_version": PIPELINE_VERSION,
    }
    request_key = (
        "request_" + hashlib.sha256(_canonical_json(identity).encode("utf-8")).hexdigest()[:16]
    )
    return {
    ...
    if resolved["aoi"]["input_type"] == "administrative_selection":
        records.append(
            _context(
                ...
                "official_context",
                "ready",
                "The resolved AOI retains dated PRG administrative-boundary provenance; it is not a facility layer.",
            )
        )
        ...
        if domain in {"public", "transport", "bridges", "water", "industrial"}:
            records.append(
                _context(
                    ...
                    "topographic_context",
                    "needs_source",
                    "A qualified BDOT10k class must be selected and verified for this AOI; building or topographic context is not facility semantics.",
                )
            )
        if domain in {"power", "water", "gas", "sewer", "telecom", "district_heating"}:
            records.append(
                _context(
                    ...
                    "reference_only",
                    "KIUT/GESUT is rendered reference imagery and cannot enter analytical GeoJSON.",
                )
            )
        records.append(
            _context(
                ...
                "reference_descriptor",
                "reference_only",
                "Orthophoto is an optional rendered reference and cannot become an object vector.",
            )
        )
        if domain == "water":
            records.append(
                _context(
                    ...
                    "derived_context",
                    "needs_source",
                    "NMT/NMPT may provide labelled raster-derived context only after a bounded source artifact is supplied.",
                )
            )
    return records
...
def _resolve_runtime_aoi(value: Any):
    ...
    if kind == "point_radius":
        ...
        longitude, latitude = (
            _finite(value["longitude"], "longitude"),
        ...
        if radius_m > MAX_CUSTOM_RADIUS_M and (longitude, latitude, radius_m) != (
            ...
            raise RuntimeRequestError(
                f"Point/radius AOI radius cannot exceed {int(MAX_CUSTOM_RADIUS_M / 1000)} km ({MAX_CUSTOM_RADIUS_M} m)"
            )
    ...
    if kind == "administrative_selection":
        if (
            set(value) != {"type", "unit_ids"}
            or not isinstance(value["unit_ids"], list)
            or not value["unit_ids"]
        ):
            raise RuntimeRequestError("Administrative AOI requires one or more unit_ids")
...
def _resolve_administrative_union(unit_ids: list[Any]):
    ...
    if area_sq_m > MAX_AREA_SQ_M:
        raise RuntimeRequestError(
            "Administrative AOI exceeds provider area limits; use a smaller county, gmina or explicit union"
        )
    return resolved
...
def _validate_administrative_selection(
    unit_ids: list[str], by_id: dict[str, dict[str, Any]]
) -> None:
    voivodeship_units = [
        uid for uid in unit_ids if by_id[uid]["properties"].get("kind") == "voivodeship"
    ]
    ...
    if len(involved_counties) > MAX_COUNTIES_SELECTION:
        raise RuntimeRequestError(
            f"Administrative selection cannot span more than {MAX_COUNTIES_SELECTION} adjacent counties"
        )

    if len(involved_counties) > 1:
        if not _are_counties_contiguous(list(involved_counties), by_id):
            raise RuntimeRequestError(
                "Selected counties and gminas must belong to directly adjacent counties"
            )
...
def _administrative_selection(unit_ids: list[Any]):
    ...
    normalized_ids = sorted(
        {_LEGACY_UNIT_ALIASES.get(unit_id, unit_id) for unit_id in requested_ids}
    )
    payload = _read_catalog()
    metadata = payload["metadata"]
    ...
    merged = unary_union(
        [_valid_polygonal_geometry(shape(by_id[unit_id]["geometry"])) for unit_id in normalized_ids]
    )
    if (
        merged.is_empty
        or not merged.is_valid
        or merged.geom_type not in {"Polygon", "MultiPolygon"}
    ):
    ...
    provenance = {
        "kind": "prg_administrative_selection",
        "source_registry_id": metadata["source_registry_id"],
        "catalog_version": metadata["catalog_version"],
        "snapshot_at": metadata["snapshot_at"],
        "source_url": metadata["source_url"],
        ...
        "fixture": "backend/data/fixtures/aoi/prg_administrative_catalog.geojson",
    }
    return _resolved(
        geometry,
...
def _valid_polygonal_geometry(geometry: Any):
    ...
    if candidate.geom_type == "GeometryCollection":
        polygonal = [
            item for item in candidate.geoms if item.geom_type in {"Polygon", "MultiPolygon"}
        ]
...
def _read_catalog() -> dict[str, Any]:
    ...
    metadata = payload["metadata"]
    if (
        metadata.get("catalog_version") != "prg_administrative_catalog/v2"
        or metadata.get("source_registry_id") != "prg_wfs"
        or metadata.get("source_crs") != WGS84
...
def _canonical_geometry(geometry: dict[str, Any]) -> dict[str, Any]:
    ...
    if geometry.get("type") == "Polygon":
        return {
            "type": "Polygon",
            "coordinates": [ring(item) for item in geometry["coordinates"]],
        }
    polygons = [
        {"type": "Polygon", "coordinates": [ring(item) for item in polygon]}
