...
COMPARISON_OUTCOMES = frozenset(
    {"matched", "conflicting", "source_only", "ambiguous", "not_comparable"}
)
...
def compare_power_features(left: dict[str, Any], right: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if any(
        ...
        or evaluate_source_eligibility(registry[source], "comparison").outcome != "allowed"
    ...
    left_type, right_type = (
        left.get("properties", {}).get("asset_type"),
        right.get("properties", {}).get("asset_type"),
    )
    if (
        not isinstance(left_type, str)
        or not isinstance(right_type, str)
        or not left.get("geometry")
        or not right.get("geometry")
    ):
    ...
    return _result(
        "matched" if distance <= 0.0001 else "source_only",
        left,
        right,
        "bounded_geometry_distance",
...
def _result(
    ...
    **extra: Any,
    ...
    return {
        ...
        "domain": "power",
        "outcome": outcome,
        "left": {"source_id": _source(left), "feature_id": _feature_id(left)},
        "right": {"source_id": _source(right), "feature_id": _feature_id(right)},
        "evidence": evidence,
        **extra,
    }
