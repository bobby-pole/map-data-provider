...
FACILITY_MAPPINGS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    "community_social": (("amenity", "community_centre"), ("amenity", "social_facility"), ("amenity", "library"), ("amenity", "arts_centre")),
}


def load_osm_public_services_fixture() -> dict[str, Any]:
    return guard_source_access("openstreetmap", "local_import", lambda: json.loads(PUBLIC_SERVICES_FIXTURE.read_text(encoding="utf-8")))
...
def categorized_osm_features() -> dict[str, list[dict[str, Any]]]:
    ...
    for feature in features:
        ...
        if category is None:
            raise ValueError("Public-services OSM fixture contains a feature without an allow-listed facility mapping")
    ...
    if any(not category_features for category_features in categorized.values()):
        ...
        raise ValueError(f"Public-services OSM fixture is missing required categories: {', '.join(missing)}")
    return categorized
...
def build_osm_public_service_layers(*, readiness: str) -> dict[str, dict[str, Any]]:
    return {
        category: normalize_analytical_vector_layer(
            ...
            metadata=public_services_osm_metadata(layer_id=f"public.{category}", readiness=readiness),
        )
...
def build_osm_public_services_cache_layer(*, readiness: str) -> dict[str, Any]:
    ...
    return normalize_analytical_vector_layer(
        {"type": "FeatureCollection", "features": features},
        metadata=public_services_osm_metadata(layer_id="public.osm_facilities", readiness=readiness),
    )
