...
class AsciiGrid:
    ...
    def bounds(self) -> tuple[float, float, float, float]:
        return (self.xllcorner, self.yllcorner, self.xllcorner + self.ncols * self.cellsize, self.yllcorner + self.nrows * self.cellsize)
...
def parse_ascii_grid(payload: bytes) -> AsciiGrid:
    ...
    try:
        ...
        cellsize, nodata_value = float(header["cellsize"]), float(header["nodata_value"])
    except ValueError as error:
        raise RasterAdapterError("ASCII Grid header values are invalid") from error
    if ncols <= 0 or nrows <= 0 or not all(math.isfinite(value) for value in (xllcorner, yllcorner, cellsize, nodata_value)) or cellsize <= 0:
...
def process_fixture(*, raster_path: Path, aoi_geometry: dict[str, Any], snapshot_at: str, source_crs: str = NMT_NMPT_SOURCE_CRS) -> dict[str, Any]:
    ...
    return guard_source_access(
        "nmt_nmpt", "local_import",
        lambda: guard_source_access("nmt_nmpt", "analytical_processing", lambda: _process(raster_path, aoi_geometry, snapshot_at)),
    )
...
def _process(raster_path: Path, aoi_geometry: dict[str, Any], snapshot_at: str) -> dict[str, Any]:
    ...
    processed = AsciiGrid(grid.ncols, grid.nrows, grid.xllcorner, grid.yllcorner, grid.cellsize, grid.nodata_value, tuple(processed_values))
    ...
    status = "valid" if aoi_coverage >= 0.999999 and valid_cells else "partial" if valid_cells else "nodata"
    return {
        ...
        "native_raster": {"format": "ascii_grid", "sha256": _digest(native_bytes), "source_crs": NMT_NMPT_SOURCE_CRS, "path": raster_path.name},
        "processed_raster": {"format": "ascii_grid", "sha256": _digest(processed_bytes), "source_crs": NMT_NMPT_SOURCE_CRS, "bytes": processed_bytes},
        "provenance": {"snapshot_at": snapshot_at, "attribution": "Główny Urząd Geodezji i Kartografii (GUGiK), NMT/NMPT", "transforms": [{"operation": "aoi_geometry_to_source_crs", "from_crs": NMT_NMPT_INTERCHANGE_CRS, "to_crs": NMT_NMPT_SOURCE_CRS, "resampling": "none"}, {"operation": "grid_clip", "resampling": "none", "cellsize": grid.cellsize}]},
        "validation": {"status": status, "crs": NMT_NMPT_SOURCE_CRS, "resolution": {"value": grid.cellsize, "unit": "metres"}, "nodata_value": grid.nodata_value, "nodata_coverage": round(nodata_coverage, 6), "aoi_coverage": round(aoi_coverage, 6), "selected_cell_count": selected_cells, "valid_cell_count": valid_cells, "grid_bounds_source_crs": list(grid.bounds)},
        "limitations": ["Raster elevations are source context, not object vectors.", "No flood risk or operational condition is inferred from elevation alone."],
    }
...
def derive_product(processed: dict[str, Any], *, product: str) -> dict[str, Any]:
    ...
    for row_index, row in enumerate(grid.values):
        for column_index, elevation in enumerate(row):
            ...
            features.append({"type": "Feature", "properties": {"derived": True, "derived_product": DERIVED_PRODUCT_VERSION, "algorithm": DERIVED_ALGORITHM, "native_raster_sha256": processed["native_raster"]["sha256"], "processed_raster_sha256": processed["processed_raster"]["sha256"], "source_crs": NMT_NMPT_SOURCE_CRS, "elevation": elevation, "unit": "metres"}, "geometry": {"type": "Point", "coordinates": [round(longitude, 7), round(latitude, 7)]}})
    return {"type": "FeatureCollection", "metadata": {"data_kind": "derived_vector", "derived": True, "derived_product": DERIVED_PRODUCT_VERSION, "algorithm": DERIVED_ALGORITHM, "native_raster_sha256": processed["native_raster"]["sha256"], "processed_raster_sha256": processed["processed_raster"]["sha256"], "limitations": ["Terrain sample points are derived context, not terrain risk or object evidence."]}, "features": features}


def serialize_ascii_grid(grid: AsciiGrid) -> bytes:
    """Serialize the exact grid resolution and nodata value deterministically."""
    header = [f"ncols {grid.ncols}", f"nrows {grid.nrows}", f"xllcorner {grid.xllcorner:g}", f"yllcorner {grid.yllcorner:g}", f"cellsize {grid.cellsize:g}", f"NODATA_value {grid.nodata_value:g}"]
...
def _cell_centre(grid: AsciiGrid, row_index: int, column_index: int) -> tuple[float, float]:
    return (grid.xllcorner + (column_index + 0.5) * grid.cellsize, grid.yllcorner + (grid.nrows - row_index - 0.5) * grid.cellsize)
