...
from geo_pipeline.extract import OVERPASS_ENDPOINTS, configure_osmnx, fetch_osm_features_geometry, sanitize_for_geojson
...
def publish_runtime_osm_collection(*, aoi: dict[str, Any], domain: str, source: dict[str, Any], query_version: str, root: Path, queried_feature_count: int | None = None, relation_elements: list[dict[str, Any]] | None = None, relation_evidence: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    relation_evidence = relation_evidence if relation_evidence is not None else _power_relation_evidence(aoi["geometry"], source, relation_elements or []) if domain == "power" else None
    manifest, files = _pack_payload(aoi=aoi, domain=domain, query_version=query_version, snapshot_at=snapshot_at, layers=layers, relation_evidence=relation_evidence)
...
def _add_industrial_categories(frame: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    if frame.empty:
        return frame
    enriched = frame.copy()
    enriched["provider_category"] = enriched.apply(lambda row: industrial_category_for_osm_feature(dict(row)) or "other", axis=1)
    return enriched[enriched["provider_category"] != "other"].copy()
...
def _clip_to_aoi(collection: dict[str, Any], geometry: dict[str, Any]) -> dict[str, Any]:
    ...
    for feature in collection["features"]:
        ...
        if candidate.is_empty:
            continue
        cb = candidate.bounds
        if candidate.geom_type == "Point" and cb[0] >= bb[0] and cb[1] >= bb[1] and cb[2] <= bb[2] and cb[3] <= bb[3]:
...
def _pack_payload(*, aoi: dict[str, Any], domain: str, query_version: str, snapshot_at: str, layers: dict[str, list[dict[str, Any]]], relation_evidence: dict[str, Any] | None = None) -> tuple[dict[str, Any], dict[str, bytes]]:
    limitations = [
    ...
    if relation_evidence is not None:
        ...
        artifacts.append({"id": "power.osm_relation_evidence", "kind": "native_vector", "format": "json", "path": relation_path, "sha256": _digest(relation_bytes), "source_provenance": provenance, "public_export": False})
...
def _fetch_power_circuit_relation_elements(geometry: dict[str, Any]) -> list[dict[str, Any]]:
    """Read bounded OSM relation membership separately from OSMnx feature geometry."""
    return guard_source_access("openstreetmap", "acquisition", lambda: _fetch_power_circuit_relation_elements_from_osm(geometry))
...
def fetch_power_circuit_relations_for_member(source_id: str) -> list[dict[str, Any]]:
    ...
    return guard_source_access("openstreetmap", "acquisition", lambda: _fetch_power_circuit_relations_for_member_from_osm(source_id))


def backfill_power_circuit_evidence_for_member(*, aoi: dict[str, Any], source_id: str, root: Path) -> dict[str, Any]:
    ...
    relation_evidence = _power_relation_evidence(
        aoi["geometry"], {"type": "FeatureCollection", "features": source_features}, fetch_power_circuit_relations_for_member(source_id)
    )
    ...
    artifacts.append({
        "id": "power.osm_relation_evidence", "kind": "native_vector", "format": "json", "path": evidence_path,
        "sha256": _digest(evidence_bytes), "source_provenance": [{"source_id": "openstreetmap", "contribution_role": "primary"}], "public_export": False,
    })
    refreshed_manifest = {**manifest, "artifacts": artifacts}
    pack = write_domain_pack(aoi["aoi_id"], "power", root=root, manifest=refreshed_manifest, files=files)
...
def _fetch_power_circuit_relations_for_member_from_osm(source_id: str) -> list[dict[str, Any]]:
    element_type, element_id = source_id.split("/", 1)
    response = requests.get(f"https://api.openstreetmap.org/api/0.6/{element_type}/{element_id}/relations", timeout=60)
    ...
    for relation in root.findall("relation"):
        ...
        for member in relation.findall("member"):
            member_type, member_ref = member.attrib.get("type"), member.attrib.get("ref")
            if member_type not in {"node", "way", "relation"}:
                continue
            try:
                members.append({"type": member_type, "ref": int(member_ref), "role": member.attrib.get("role", "")})
...
def _fetch_power_circuit_relation_elements_from_osm(geometry: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    blocks = "".join(f'relation["power"="circuit"](poly:"{_overpass_polygon(polygon)}");' for polygon in polygons)
...
def _overpass_polygon(polygon: Any) -> str:
    return " ".join(f"{latitude:.7f} {longitude:.7f}" for longitude, latitude in polygon.exterior.coords)


def _power_relation_evidence(geometry: dict[str, Any], source: dict[str, Any], elements: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    delivered = {
        ...
        if isinstance(feature, dict) and isinstance(feature.get("properties"), dict) and _source_id(feature["properties"])
    }
    ways = {f"way/{element['id']}": element for element in elements if element.get("type") == "way" and isinstance(element.get("id"), int)}
    relations = []
    for relation in elements:
        ...
        if not isinstance(tags, dict) or tags.get("power") != "circuit" or not isinstance(members, list):
            continue
        retained = []
        for member in members:
            if not isinstance(member, dict) or member.get("type") not in {"node", "way", "relation"} or not isinstance(member.get("ref"), int):
                continue
            source_id = f"{member['type']}/{member['ref']}"
            feature = delivered.get(source_id)
            if feature is None:
                continue
            record: dict[str, Any] = {"source_id": source_id, "role": str(member.get("role") or "member")}
            if member["type"] == "way":
                way = ways.get(source_id)
                node_ids = way.get("nodes") if isinstance(way, dict) else None
                if isinstance(node_ids, list) and len(node_ids) >= 2 and isinstance(node_ids[0], int) and isinstance(node_ids[-1], int):
                    record["endpoint_evidence"] = {"start": f"node/{node_ids[0]}", "end": f"node/{node_ids[-1]}"}
            feature_geometry = feature.get("geometry")
            if isinstance(feature_geometry, dict) and feature_geometry.get("type") == "LineString" and isinstance(feature_geometry.get("coordinates"), list) and len(feature_geometry["coordinates"]) >= 2:
                record["geometry"] = feature_geometry
            elif member["type"] == "way":
                record["availability"] = "Member is delivered in the AOI but has no retained LineString geometry."
        ...
        if retained:
            relations.append({
                ...
                "limitations": ["Only OSM circuit members delivered inside this bounded AOI are retained.", "No topology, connectivity, flow, outage or cascade is inferred."],
                "members": retained,
            })
    ...
    checksum_source = json.dumps(elements, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return {
...
def _unavailable_power_relation_evidence(geometry: dict[str, Any], error: Exception) -> dict[str, Any]:
    boundary = shape(geometry).bounds
    return {
        "relation_evidence_version": "osm_power_relation_evidence/v2",
        "source": "OpenStreetMap",
...
def _source_id(properties: dict[str, Any]) -> str | None:
    ...
    return source_id if isinstance(source_id, str) and re.fullmatch(r"(?:node|way|relation)/[1-9][0-9]*", source_id) else None
