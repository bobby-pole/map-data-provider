...
def parse_capabilities(payload: bytes) -> OrthophotoLayer:
    ...
    layer = next((item for item in root.iter() if _local_name(item.tag) == "Layer" and _text(item, "Name") == ORTHOPHOTO_LAYER), None)
    if layer is None:
        raise OrthophotoAdapterError("Orthophoto capabilities schema drift: missing Raster layer")
    crs_values = {child.text.strip() for child in layer if _local_name(child.tag) == "CRS" and child.text}
    ...
    return OrthophotoLayer(ORTHOPHOTO_LAYER, _text(layer, "Title") or ORTHOPHOTO_LAYER, bounds, metadata_url)


def reference_descriptor(*, aoi_geometry: dict[str, Any], capabilities: bytes | None) -> dict[str, Any]:
    """Return metadata for a reference background; never return imagery or vectors."""
    return guard_source_access("geoportal_orthophoto", "reference", lambda: _descriptor(aoi_geometry, capabilities))
...
def _descriptor(aoi_geometry: dict[str, Any], capabilities: bytes | None) -> dict[str, Any]:
    ...
    if capabilities is None:
        return {**base, "status": "service_unavailable", "coverage": {"state": "unknown"}, "get_map": None, "metadata_url": None}
    ...
    return {
        **base,
        "status": status,
        "title": raster.title,
        "coverage": {"state": "possible" if status == "available_reference" else "uncovered", "bbox_wgs84": list(raster.bbox_wgs84)},
...
def build_getmap_url(bounds_wgs84: tuple[float, float, float, float]) -> str:
    ...
    if len(bounds_wgs84) != 4 or not all(isinstance(value, (int, float)) and not isinstance(value, bool) and math.isfinite(value) for value in bounds_wgs84):
    ...
    params = {
        "SERVICE": "WMS", "VERSION": ORTHOPHOTO_WMS_VERSION, "REQUEST": "GetMap", "LAYERS": ORTHOPHOTO_LAYER,
...
def _metadata_url(layer: ET.Element) -> str:
    resource = next((item for item in layer.iter() if _local_name(item.tag) == "OnlineResource"), None)
    value = resource.attrib.get("{http://www.w3.org/1999/xlink}href") if resource is not None else None
...
def _is_expected_metadata_url(value: str) -> bool:
    ...
    return parsed.scheme in {"http", "https"} and parsed.hostname == ORTHOPHOTO_METADATA_HOST and parsed.path.startswith("/wss/service/")
