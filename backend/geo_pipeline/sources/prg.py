...
PRG_FEATURE_TYPES = {
    "ms:A01_Granice_wojewodztw": {"source_class": "wojewodztwo", "normalized_role": "administrative_boundary", "boundary": True},
    "ms:A02_Granice_powiatow": {"source_class": "powiat", "normalized_role": "administrative_boundary", "boundary": True},
    "ms:A03_Granice_gmin": {"source_class": "gmina", "normalized_role": "administrative_boundary", "boundary": True},
    "ms:K01_Komenda_wojewodzka_policji": {"source_class": "police_command_voivodeship", "normalized_role": "official_public_service", "boundary": False},
    "ms:K02_Komenda_powiatowa_policji": {"source_class": "police_command_county", "normalized_role": "official_public_service", "boundary": False},
    "ms:K03_Komenda_stoleczna_policji": {"source_class": "police_command_capital", "normalized_role": "official_public_service", "boundary": False},
    "ms:K04_Komenda_rejonowa_policji": {"source_class": "police_command_district", "normalized_role": "official_public_service", "boundary": False},
    "ms:K05_Komisariat_policji": {"source_class": "police_station", "normalized_role": "official_public_service", "boundary": False},
    "ms:K06_Komenda_wojewodzka_strazy_pozarnej": {"source_class": "fire_command_voivodeship", "normalized_role": "official_public_service", "boundary": False},
    "ms:K07_Komenda_powiatowa_strazy_pozarnej": {"source_class": "fire_command_county", "normalized_role": "official_public_service", "boundary": False},
}
...
def build_getfeature_url(feature_type: str, *, bbox_2180: tuple[float, float, float, float] | None = None) -> str:
    ...
    params: dict[str, str] = {
        ...
        "SRSNAME": PRG_SOURCE_CRS, "OUTPUTFORMAT": "application/gml+xml; version=3.2",
    }
...
def fetch_gml(feature_type: str, request: Callable[[str], bytes], *, bbox_2180: tuple[float, float, float, float] | None = None) -> bytes:
...
def capability_feature_types(payload: bytes) -> set[str]:
    ...
    names = {
        ...
        if _local_name(element.tag) == "Name" and isinstance(element.text, str) and element.text.strip().startswith("ms:")
    }
    return names
...
def inspect_fixture(*, feature_type: str, capabilities: bytes, schema: bytes, gml: bytes | None, snapshot_at: str) -> PrgAdapterOutcome:
    ...
    if feature_type not in capability_feature_types(capabilities):
        return _outcome("service_unavailable", feature_type, 0, snapshot_at, reason="feature_type_not_advertised")
    fields = schema_field_names(schema)
    required = {"msGeometry", "JPT_NAZWA_", "IIP_IDENTY", "JPT_ID"}
    if not required <= fields:
        return _outcome("schema_drift", feature_type, 0, snapshot_at, missing_fields=sorted(required - fields))
    if gml is None:
        return _outcome("service_unavailable", feature_type, 0, snapshot_at, reason="fixture_response_missing")
...
def normalize_gml(feature_type: str, payload: bytes, *, snapshot_at: str) -> dict[str, Any]:
    ...
    return guard_source_access(
        "prg_wfs", "analytical_processing",
        lambda: _normalize_gml(feature_type, definition, payload, snapshot_at),
    )


def clip_non_boundary_features(collection: dict[str, Any], aoi_geometry: dict[str, Any]) -> dict[str, Any]:
    ...
    for feature in collection.get("features", []):
        ...
        if not geometry.is_empty:
            clipped.append({**feature, "geometry": json.loads(json.dumps(geometry_mapping(geometry)))})
    return {**collection, "features": clipped}


def _normalize_gml(feature_type: str, definition: dict[str, Any], payload: bytes, snapshot_at: str) -> dict[str, Any]:
    try:
        root = ET.fromstring(payload)
    except ET.ParseError as error:
    ...
    for element in root.iter():
        ...
        raw = {_local_name(child.tag): (child.text or "").strip() for child in element if _local_name(child.tag) != "msGeometry"}
        ...
        features.append({
            ...
            "properties": {
                ...
                "limitations": ["PRG feature presence is not a completeness or operational-status guarantee."],
            },
            "geometry": geometry,
        })
    return {"type": "FeatureCollection", "metadata": {"source_registry_id": "prg_wfs", "feature_type": feature_type, "source_crs": PRG_SOURCE_CRS, "snapshot_at": snapshot_at}, "features": features}
...
def _geometry(element: ET.Element) -> dict[str, Any] | None:
    ...
    if polygon is not None:
        ...
        if len(values) < 8 or len(values) % 2:
            return None
        coordinates = [_to_wgs84(values[index], values[index + 1]) for index in range(0, len(values), 2)]
...
def _outcome(status: str, feature_type: str, feature_count: int, snapshot_at: str, **extra: Any) -> PrgAdapterOutcome:
    return PrgAdapterOutcome(status=status, feature_type=feature_type, feature_count=feature_count, evidence={"adapter_version": PRG_ADAPTER_VERSION, "source_registry_id": "prg_wfs", "snapshot_at": snapshot_at, **extra})
