...
def main() -> None:
    ...
    for item in supports["elements"]:
        tags = item.get("tags", {})
        kind = tags.get("power") or (
            "utility_pole" if tags.get("man_made") == "utility_pole" else None
        )
        if kind not in {"tower", "pole", "portal", "utility_pole"}:
            continue
        projected = {key: tags[key] for key in TAGS if key in tags}
        features.append(
            {
                "type": "Feature",
                "properties": {"element": "node", "id": item["id"], **projected},
                "geometry": {
                    "type": "Point",
    ...
    fc = {
        ...
        "metadata": {
            ...
            "source_query": "Captured bounded Overpass support snapshot for Rybnik 35 km AOI.",
            "source_url": "https://overpass-api.de/api/interpreter",
            "attribution": "© OpenStreetMap contributors",
            "license": "ODbL-1.0",
            "bbox": BBOX,
            "source_checksum": digest(supports_path),
            "coverage": "bounded_aoi_snapshot",
        },
        "features": features,
    }
    (OUT / "osm-power-supports-full.geojson").write_text(
        ...
        for member in relation.get("members", []):
            ...
            if member["type"] == "way" and (way := elements.get(("way", member["ref"]))):
                ...
                if len(coords) > 1:
                    ...
                    record["availability"] = "missing_member_geometry"
                if len(member_nodes) > 1:
                    record["endpoint_evidence"] = {
                        "start": f"node/{member_nodes[0]}",
                        "end": f"node/{member_nodes[-1]}",
                    }
                ...
                for node_id in way.get("nodes", []):
                    if node_id in nodes:
                        reverse.setdefault(f"node/{node_id}", []).append(
                            f"relation/{relation['id']}"
                        )
            else:
                record["availability"] = "missing_member_geometry"
            members.append(record)
            reverse.setdefault(source_id, []).append(f"relation/{relation['id']}")
        result.append(
            {
                ...
                "members": members,
                "limitations": [
                    "Only captured OSM members and geometry are shown.",
                    "No connectivity, flow, outage or cascade model is implied.",
                ],
            }
        )
    payload = {
        ...
        "snapshot_at": snapshot_at,
        "bbox": BBOX,
        "source_checksum": digest(circuits_path),
        "relations": result,
        "reverse_member_index": reverse,
    }
    (OUT / "osm-power-circuit-evidence.json").write_text(
    ...
    records = {
        f"{item['type']}/{item['id']}": {
            key: tags[key] for key in TAGS if key in (tags := item.get("tags", {}))
        }
        for item in tags_payload["elements"]
        if item["type"] in {"node", "way", "relation"} and item.get("tags")
    }
    attributes = {
        "attribute_evidence_version": "osm_power_attributes/v1",
        "source": "OpenStreetMap",
        "snapshot_at": tag_snapshot_at,
        "bbox": BBOX,
        "source_checksum": digest(tags_path),
        "attributes": records,
    }
    (OUT / "osm-power-attributes.json").write_text(
        json.dumps(attributes, ensure_ascii=False), encoding="utf-8"
    )
