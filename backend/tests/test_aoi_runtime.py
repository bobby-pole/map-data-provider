...
def test_runtime_telecom_publication_preserves_empty_line_source_gap(tmp_path) -> None:
    aoi = resolve_runtime_request({"aoi": {"type": "administrative_selection", "unit_ids": ["county_rybnik_city"]}, "profiles": ["telecom"]})["aoi"]
    source = {"type": "FeatureCollection", "features": [
        {"type": "Feature", "properties": {"element": "node", "id": 1, "provider_category": "towers", "man_made": "mast", "tower:type": "communication"}, "geometry": {"type": "Point", "coordinates": [18.5, 50.1]}},
        {"type": "Feature", "properties": {"element": "node", "id": 2, "provider_category": "facilities", "telecom": "exchange"}, "geometry": {"type": "Point", "coordinates": [18.51, 50.11]}},
    ]}

    result = publish_runtime_osm_collection(aoi=aoi, domain="telecom", source=source, query_version=TELECOM_OSM_QUERY.query_version, root=tmp_path)
    pack = read_domain_pack(aoi["aoi_id"], "telecom", root=tmp_path)
    artifacts = {artifact["id"]: artifact for artifact in pack["artifacts"]}
    lines = json.loads((tmp_path / aoi["aoi_id"] / "telecom" / "domain-pack-v2" / artifacts["telecom.lines"]["path"]).read_text())
