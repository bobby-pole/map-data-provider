...
def test_bridges_domain_pack_builds_independent_categories_and_inspection_points(
    tmp_path: Path,
) -> None:
    ...
    bridges = json.loads(
        (pack_root / artifacts["bridges.bridges"]["path"]).read_text(encoding="utf-8")
    )
    viaducts = json.loads(
    ...
    context = json.loads(
        (pack_root / artifacts["bridges.context_and_comparison"]["path"]).read_text(
            encoding="utf-8"
        )
    )
    ...
    public = read_domain_pack("rybnik_35km", "bridges", root=tmp_path, public_export=True)[
        "artifacts"
    ]
