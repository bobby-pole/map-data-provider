...
from geo_pipeline.domain_pack import build_rybnik_district_heating_domain_pack, read_domain_pack
...
def test_district_heating_requires_explicit_semantics() -> None:
    ...
    assert category_for_osm_feature({"power": "generator", "generator:output:heat": "true"}) == "plants"
...
def test_district_heating_query_is_bounded_to_explicit_candidates() -> None:
    ...
    assert DISTRICT_HEATING_OSM_QUERY.tags["plant:output:heat"] == ["yes", "true", "1", "heat"]
    assert DISTRICT_HEATING_OSM_QUERY.tags["substance"] == ["hot_water", "steam", "heat"]
