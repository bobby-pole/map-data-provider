...
def test_power_domain_pack_v2_preserves_v1_cache_compatibility(tmp_path: Path) -> None:
    ...
    public = read_domain_pack("rybnik_35km", "power", root=tmp_path, public_export=True)["artifacts"]
...
def test_industrial_domain_pack_synthesizes_industrial_facilities(tmp_path: Path) -> None:
    ...
    presentation = json.loads((tmp_path / "rybnik_35km" / "industrial" / "domain-pack-v2" / "presentation" / "manifest.json").read_text())
...
def test_emergency_pack_retains_original_osm_geometry_and_distinct_prg_representative_points(tmp_path: Path) -> None:
    ...
    assert hospital["features"][0]["geometry"]["type"] in ("Point", "Polygon", "MultiPolygon")
    assert inspection["features"][0]["properties"]["source_geometry_type"] in ("Point", "Polygon", "MultiPolygon", "LineString")
    assert official_police["features"][0]["properties"] == {
        ...
        "source_response_sha256": "cd3e2eb355292207aa72b71cc5c3e29328fd7d48261c17288e5d87e7264f5266", "source_attributes": {
            ...
            "jpt_id": "1350186", "version_from": "2025-07-17T00:00:00Z",
        },
    }
    assert official_police["features"][0]["geometry"]["type"] == "Point"
    assert read_domain_pack("rybnik_35km", "emergency", root=tmp_path, public_export=True)["artifacts"][-1]["id"] == "emergency.inspection_points"
...
def test_public_pack_keeps_facility_semantics_separate_from_buildings_and_publishes_comparison_evidence(tmp_path: Path) -> None:
    ...
    assert administration["features"][0]["geometry"]["type"] in ("Point", "Polygon", "MultiPolygon")
    ...
    assert {artifact["id"] for artifact in read_domain_pack("rybnik_35km", "public", root=tmp_path, public_export=True)["artifacts"]} == {
...
def test_transport_pack_keeps_semantics_separate_and_publishes_comparison_evidence(tmp_path: Path) -> None:
    ...
    assert {artifact["id"] for artifact in read_domain_pack("rybnik_35km", "transport", root=tmp_path, public_export=True)["artifacts"]} == {
...
def test_gas_pack_keeps_semantics_separate_and_publishes_comparison_evidence(tmp_path: Path) -> None:
    ...
    assert inspection["features"][0]["properties"]["origin_artifact"] in ("gas.facilities", "gas.pipelines")
    assert {artifact["id"] for artifact in read_domain_pack("rybnik_35km", "gas", root=tmp_path, public_export=True)["artifacts"]} == {
...
def test_sewer_pack_keeps_semantics_separate_and_publishes_comparison_evidence(tmp_path: Path) -> None:
    ...
    assert pipelines["features"][0]["geometry"]["type"] in ("LineString", "MultiLineString")
    assert inspection["features"][0]["properties"]["origin_artifact"] in ("sewer.facilities", "sewer.pipelines")
    assert {artifact["id"] for artifact in read_domain_pack("rybnik_35km", "sewer", root=tmp_path, public_export=True)["artifacts"]} == {
