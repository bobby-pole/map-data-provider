...
def test_power_domain_pack_v2_preserves_v1_cache_compatibility(tmp_path: Path) -> None:
    ...
    pack_root = tmp_path / "rybnik_35km" / "power" / "domain-pack-v2"
    representative_points = json.loads(
        (pack_root / artifacts["power.representative_points"]["path"]).read_text()
    )
    ...
    assert (
        artifacts["power.kiut_reference"]["public_export"] is False
        and "path" not in artifacts["power.kiut_reference"]
    )
    assert artifacts["power.osm_source_evidence"]["format"] == "json"
    assert representative_points["features"][0]["properties"]["source_id"] == "way/39480264"
    assert (
        representative_points["features"][0]["properties"]["source_geometry_type"] == "LineString"
    )
    ...
    public = read_domain_pack("rybnik_35km", "power", root=tmp_path, public_export=True)[
        "artifacts"
    ]
...
def test_industrial_domain_pack_synthesizes_industrial_facilities(
    tmp_path: Path,
) -> None:
    ...
    assert {
        ...
        "industrial.works",
        "industrial.building_context",
        "industrial.military_context",
        "industrial.inspection_points",
        "industrial.osm_source_evidence",
        "industrial.context_and_comparison",
    } == set(artifacts)
...
def test_domain_pack_rejects_escape_checksum_and_count_mismatch(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="escapes"):
        read_domain_pack("rybnik_35km", "power", root=tmp_path)


def test_domain_pack_keeps_native_raster_private_and_rejects_reference_public_export(
    tmp_path: Path,
) -> None:
        ...
        "artifacts": [
            {
                ...
                "kind": "native_raster",
                "format": "geotiff",
                "path": "rasters/terrain.tif",
                "sha256": "b06649195b8f4332ac1430d51d9fba43f94acd5189b47a4d875e8e1a4a9479dd",
                "source_provenance": [
                    {"source_id": "openstreetmap", "contribution_role": "primary"}
                ],
                "public_export": False,
...
def test_emergency_pack_keeps_prg_representative_points_private_until_redistribution_terms_are_recorded(
    ...
    pack_root = tmp_path / "rybnik_35km" / "emergency" / "domain-pack-v2"
    artifacts = {artifact["id"]: artifact for artifact in pack["artifacts"]}
    hospital = json.loads((pack_root / artifacts["emergency.hospital"]["path"]).read_text())
    inspection = json.loads(
        (pack_root / artifacts["emergency.inspection_points"]["path"]).read_text()
    )
    official_police = json.loads(
        (pack_root / artifacts["emergency.official_police"]["path"]).read_text()
    )

    assert hospital["features"][0]["geometry"]["type"] in (
        "Point",
        "Polygon",
        "MultiPolygon",
    )
    assert inspection["features"][0]["properties"]["source_geometry_type"] in (
        "Point",
        "Polygon",
        "MultiPolygon",
        "LineString",
    ...
    assert official_police["features"][0]["properties"] == {
        ...
        "confidence": "medium",
        "missing_fields": [],
        "limitations": official_police["metadata"]["limitations"],
        "eligible_for_analysis": True,
        "source_geometry_type": "MultiSurface",
        "geometry_role": "representative_point_from_official_unit_area",
        "source_response_sha256": "cd3e2eb355292207aa72b71cc5c3e29328fd7d48261c17288e5d87e7264f5266",
        "source_attributes": {
            "name": "Komenda Miejska Policji w Żorach",
            "official_type": "K02_Komenda_powiatowa_policji",
            "iip_identifier": "beaf5604-b69f-40f9-bfd9-cdfd23fb30b4",
            "jpt_id": "1350186",
            "version_from": "2025-07-17T00:00:00Z",
        },
    }
    ...
    assert (
        ...
        == "emergency.inspection_points"
    )


def test_public_pack_keeps_facility_semantics_separate_from_buildings_and_publishes_comparison_evidence(
    tmp_path: Path,
) -> None:
    ...
    pack_root = tmp_path / "rybnik_35km" / "public" / "domain-pack-v2"
    artifacts = {artifact["id"]: artifact for artifact in pack["artifacts"]}
    administration = json.loads(
        (pack_root / artifacts["public.administration"]["path"]).read_text()
    )
    inspection = json.loads((pack_root / artifacts["public.inspection_points"]["path"]).read_text())
    context = json.loads(
        (pack_root / artifacts["public.context_and_comparison"]["path"]).read_text()
    )

    assert administration["features"][0]["geometry"]["type"] in (
        "Point",
        "Polygon",
        "MultiPolygon",
    )
    assert inspection["features"][0]["properties"]["origin_artifact"].startswith("public.")
    assert inspection["features"][0]["properties"]["source_geometry_type"] in (
        "Point",
        "Polygon",
        "MultiPolygon",
    ...
    assert {
        artifact["id"]
        for artifact in read_domain_pack(
            "rybnik_35km", "public", root=tmp_path, public_export=True
        )["artifacts"]
    } == {
...
def test_transport_pack_keeps_semantics_separate_and_publishes_comparison_evidence(
    tmp_path: Path,
) -> None:
    ...
    inspection = json.loads(
        (pack_root / artifacts["transport.inspection_points"]["path"]).read_text()
    )
    ...
    assert {
        artifact["id"]
        for artifact in read_domain_pack(
            "rybnik_35km", "transport", root=tmp_path, public_export=True
        )["artifacts"]
    } == {
        "transport.roads",
        "transport.railways",
...
def test_gas_pack_keeps_semantics_separate_and_publishes_comparison_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert pipelines["features"][0]["geometry"]["type"] in (
        "LineString",
        "MultiLineString",
    )
    assert inspection["features"][0]["properties"]["origin_artifact"] in (
        "gas.facilities",
        "gas.pipelines",
    )
    assert {
        artifact["id"]
        for artifact in read_domain_pack("rybnik_35km", "gas", root=tmp_path, public_export=True)[
            "artifacts"
        ]
    } == {
        "gas.facilities",
...
def test_sewer_pack_keeps_semantics_separate_and_publishes_comparison_evidence(
    tmp_path: Path,
) -> None:
    ...
    assert {
        artifact["id"]
        for artifact in read_domain_pack("rybnik_35km", "sewer", root=tmp_path, public_export=True)[
            "artifacts"
        ]
    } == {
        "sewer.facilities",
