...
def test_gas_domain_pack_builds_independent_categories_and_inspection_points(
    tmp_path: Path,
) -> None:
    ...
    facilities = json.loads(
        (pack_root / artifacts["gas.facilities"]["path"]).read_text(encoding="utf-8")
    )
    pipelines = json.loads(
        (pack_root / artifacts["gas.pipelines"]["path"]).read_text(encoding="utf-8")
    )
    inspection = json.loads(
        (pack_root / artifacts["gas.inspection_points"]["path"]).read_text(encoding="utf-8")
    )
    context = json.loads(
        (pack_root / artifacts["gas.context_and_comparison"]["path"]).read_text(encoding="utf-8")
    )
    ...
    evidence = json.loads(
        (pack_root / artifacts["gas.osm_source_evidence"]["path"]).read_text(encoding="utf-8")
    )
    assert evidence["category_rules"]["gas.facilities"].endswith("substance=gas.")
    assert evidence["category_rules"]["gas.pipelines"].startswith(
        "man_made=pipeline requires substance=gas"
    )
