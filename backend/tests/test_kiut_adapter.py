...
from geo_pipeline.sources.kiut import (
    KiutAdapterError,
    build_getmap_url,
    parse_capabilities,
...
CAPABILITIES = (
    Path(__file__).resolve().parents[1] / "data" / "fixtures" / "kiut" / "capabilities.xml"
).read_bytes()
AOI = {
    "type": "Polygon",
...
@pytest.mark.parametrize(
    "domain", ["power", "water", "gas", "sewer", "telecom", "district_heating"]
)
def test_reference_descriptors_are_non_analytical_and_allow_list_domains(
    domain: str,
) -> None:
...
def test_reports_uncovered_and_unsupported_scale_without_vector_fallback() -> None:
    ...
    assert (
        reference_descriptor(
            domain="power", aoi_geometry=AOI, scale_denominator=1000, capabilities=None
        )["status"]
        == "service_unavailable"
    )
...
def test_rejects_schema_drift_and_arbitrary_wms_layer_urls() -> None:
    with pytest.raises(KiutAdapterError, match="schema drift"):
        parse_capabilities(
            b'<WMS_Capabilities version="1.3.0"><Capability><Layer /></Capability></WMS_Capabilities>'
        )
