...
def test_native_and_processed_rasters_keep_checksums_provenance_and_resolution() -> None:
    result = process_fixture(
        raster_path=FIXTURES / "valid.asc",
        aoi_geometry=_aoi((500000, 250000, 500030, 250030)),
        snapshot_at="2026-08-02T00:00:00Z",
    )
    assert result["validation"] == {
        "status": "valid",
        "crs": "EPSG:2180",
        "resolution": {"value": 10.0, "unit": "metres"},
        "nodata_value": -9999.0,
        "nodata_coverage": 0.0,
        "aoi_coverage": 1.0,
...
def test_reports_partial_and_nodata_aoi_coverage_without_claiming_risk() -> None:
    partial = process_fixture(
        raster_path=FIXTURES / "partial.asc",
        aoi_geometry=_aoi((500000, 250000, 500030, 250030)),
        snapshot_at="2026-08-02T00:00:00Z",
    )
    assert partial["validation"]["status"] == "partial"
    assert 0 < partial["validation"]["aoi_coverage"] < 1
    nodata = process_fixture(
        raster_path=FIXTURES / "nodata.asc",
        aoi_geometry=_aoi((500000, 250000, 500030, 250030)),
        snapshot_at="2026-08-02T00:00:00Z",
    )
...
def test_rejects_corrupt_empty_or_incompatible_raster_inputs() -> None:
    ...
    with pytest.raises(RasterAdapterError, match="does not cover"):
        process_fixture(
            raster_path=FIXTURES / "valid.asc",
            aoi_geometry=_aoi((600000, 350000, 600010, 350010)),
            snapshot_at="2026-08-02T00:00:00Z",
        )
