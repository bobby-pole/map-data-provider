...
def test_power_support_snapshot_is_full_bounded_and_classified() -> None:
    snapshot = json.loads(
        (FIXTURE_ROOT / "osm-power-supports-full.geojson").read_text(encoding="utf-8")
    )
    ...
    classes = {
        feature["properties"].get("power") or feature["properties"].get("man_made")
...
def test_circuit_snapshot_has_deterministic_member_and_node_reverse_indexes() -> None:
    evidence = json.loads(
        (FIXTURE_ROOT / "osm-power-circuit-evidence.json").read_text(encoding="utf-8")
    )
...
def test_power_attribute_snapshot_preserves_verified_plant_reference_links() -> None:
    attributes = json.loads(
        (FIXTURE_ROOT / "osm-power-attributes.json").read_text(encoding="utf-8")
    )
