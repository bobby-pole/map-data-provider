...
def test_category_for_osm_feature_normalizes_sewer_tags_and_rejects_unmapped() -> None:
    ...
    assert (
        category_for_osm_feature({"man_made": "pumping_station", "pumping": "sewer"})
        == "facilities"
    )
    assert (
        category_for_osm_feature({"man_made": "pumping_station", "substance": "wastewater"})
        == "facilities"
    )
    ...
    assert (
        category_for_osm_feature({"man_made": "pipeline", "substance": "sewerage"}) == "pipelines"
    )
...
def test_sewer_domain_pack_builds_independent_categories_and_inspection_points(
    tmp_path: Path,
) -> None:
    ...
    facilities = json.loads(
        (pack_root / artifacts["sewer.facilities"]["path"]).read_text(encoding="utf-8")
    )
    pipelines = json.loads(
        (pack_root / artifacts["sewer.pipelines"]["path"]).read_text(encoding="utf-8")
    )
    inspection = json.loads(
        (pack_root / artifacts["sewer.inspection_points"]["path"]).read_text(encoding="utf-8")
    )
    context = json.loads(
        (pack_root / artifacts["sewer.context_and_comparison"]["path"]).read_text(encoding="utf-8")
    )
    ...
    evidence = json.loads(
        (pack_root / artifacts["sewer.osm_source_evidence"]["path"]).read_text(encoding="utf-8")
    )
    assert "stormwater" in evidence["sewer_semantics_rule"]

    public = read_domain_pack("rybnik_35km", "sewer", root=tmp_path, public_export=True)[
        "artifacts"
    ]
