...
def test_category_for_osm_feature_requires_explicit_telecom_semantics() -> None:
    ...
    assert category_for_osm_feature({"man_made": "tower", "communication:mobile_phone": "yes"}) == "towers"
    assert category_for_osm_feature({"telecom": "exchange"}) == "facilities"
    assert category_for_osm_feature({"man_made": "antenna", "communication:radio": "yes"}) == "facilities"
...
def test_telecom_domain_pack_keeps_missing_lines_as_visible_source_gap(tmp_path: Path) -> None:
    ...
    evidence = json.loads((pack_root / artifacts["telecom.osm_source_evidence"]["path"]).read_text(encoding="utf-8"))
