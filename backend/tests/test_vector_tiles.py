...
from geo_pipeline.vector_tiles import (
    _tiles_for_bounds,
    _voltage_style,
    build_map_presentation,
    read_map_presentation,
)
...
def _layer(layer_id: str, feature: dict) -> dict:
    return {
        ...
        "metadata": {
            ...
            "limitations": ["Fixture-only MVT evidence."],
            "feature_count": 1,
        },
        "features": [feature],
    }
    ...
    lines = _layer(
        "power.lines",
        {
            ...
            "properties": {
                ...
                "confidence": "medium",
                "limitations": ["Fixture-only MVT evidence."],
                "osm_tags": {"voltage": "220000"},
            },
            "geometry": {
                "type": "LineString",
                "coordinates": [[18.45, 50.1], [18.55, 50.2]],
            },
        },
    )
    assets = _layer(
...
def test_public_layers_build_deterministic_mvt_pmtiles_with_compact_properties(
    tmp_path: Path,
) -> None:
    ...
    assert {layer["artifact_id"] for layer in first["layers"]} == {
        "power.lines",
        "power.assets",
    }
    ...
    assert reader.metadata()["vector_layers"][0]["id"] in {
        "power_lines",
        "power_assets",
    }
    ...
    properties = next(
        ...
        if feature["properties"]["source_id"] == "way/1"
    )
    ...
    assert read_map_presentation(pack_root=pack_root, manifest=manifest)["archive"][
        "size_bytes"
    ] == len(first_bytes)
...
def test_presentation_rejects_reference_only_or_stale_inputs(tmp_path: Path) -> None:
    pack_root, manifest = _fixture_pack(tmp_path)
    manifest["artifacts"][0]["source_provenance"] = [
...
def test_voltage_states_and_power_support_zoom_thresholds_are_explicit() -> None:
    ...
    assert _voltage_style({"voltage": "110000;220000"}) == (
        "multiple_voltage",
        "high_220",
    )
    ...
    bounds = (18.5, 50.1, 18.5, 50.1)
    assert min(coordinate[0] for coordinate in _tiles_for_bounds(bounds, min_zoom=12)) == 12
    assert min(coordinate[0] for coordinate in _tiles_for_bounds(bounds, min_zoom=14)) == 14
