...
def test_fixture_worker_creates_complete_cache_without_network(tmp_path: Path) -> None:
    ...
    assert read_cached_layer(cache_paths("rybnik_35km", "power", root=tmp_path))["readiness"]["readiness"] == "usable_with_limitations"
...
def test_emergency_fixture_worker_keeps_official_and_community_evidence_separate(tmp_path: Path) -> None:
    result = run_worker(aoi="rybnik_35km", domain="emergency", input_mode="fixture", cache_root=tmp_path)
...
def test_public_fixture_worker_publishes_independent_semantic_categories(tmp_path: Path) -> None:
    result = run_worker(aoi="rybnik_35km", domain="public", input_mode="fixture", cache_root=tmp_path)
...
def test_sewer_fixture_worker_publishes_the_versioned_explicit_semantics_profile(tmp_path: Path) -> None:
    result = run_worker(aoi="rybnik_35km", domain="sewer", input_mode="fixture", cache_root=tmp_path)
...
def test_telecom_fixture_worker_keeps_kiut_reference_only_and_publishes_line_gap(tmp_path: Path) -> None:
    result = run_worker(aoi="rybnik_35km", domain="telecom", input_mode="fixture", cache_root=tmp_path)
...
def test_partial_adapter_output_does_not_replace_a_valid_cache(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(Exception) as error:
        worker.run_worker(aoi="rybnik_35km", domain="power", input_mode="fixture", cache_root=tmp_path)
    ...
    assert read_domain_pack("rybnik_35km", "power", root=tmp_path)["domain_pack_version"] == "provider_domain_pack/v2"
...
def test_refresh_live_runtime_outcomes_isolates_domain_refresh_failure(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    progress: list[dict] = []
    res = worker._refresh_live_runtime_outcomes(resolved, outcomes, tmp_path, progress=progress.append, executor_type=ThreadPoolExecutor)

    assert len(res) == 2
    assert [(event["event"], event["completed_domains"], event["total_domains"]) for event in progress[:2]] == [
        ("domain_started", 0, 2), ("domain_started", 0, 2),
    ]
    assert [event["completed_domains"] for event in progress if event["event"] == "domain_completed"] == [1, 2]
...
def test_partial_runtime_snapshot_retries_only_domains_that_failed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    first = worker.run_runtime_worker(request=request, input_mode="live", cache_root=CACHE_DIR, runtime_root=tmp_path)
    second = worker.run_runtime_worker(request=request, input_mode="live", cache_root=CACHE_DIR, runtime_root=tmp_path)

    assert first["request_result"] == "refresh"
    assert next(outcome for outcome in first["outcomes"] if outcome["domain"] == "water")["status"] == "failed"
...
def test_domain_timeout_scales_with_disconnected_administrative_aoi_parts() -> None:
    polygon = {"type": "Polygon", "coordinates": [[[18.0, 50.0], [18.1, 50.0], [18.1, 50.1], [18.0, 50.0]]]}
    multi_polygon = {
        "type": "MultiPolygon",
        "coordinates": [
