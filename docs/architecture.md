...
| PRG                  | vector / `wfs_gml`            | Official source evidence; public export is blocked until product-specific redistribution terms are recorded.  |
| BDOT10k              | vector / `gpkg_geoparquet`    | Official package source evidence; public export is blocked until package terms are recorded.                  |
...
Every analytical cache record references its registry ID and retains `source_url`, `source_query`, `snapshot_at`, `pipeline_version` and `query_version`. The existing `rybnik_35km/power` cache keeps its v1 provenance shape and resolves it through the v2 OpenStreetMap record. Before a source is acquired, locally imported, processed analytically or exported, the provider evaluates that requested use against the registry. Only qualified free sources pass acquisition/import; analytical processing also requires an analytical, analysis-eligible non-rendered source; public export additionally requires explicit distribution permission. PRG and BDOT10k are currently blocked from public export pending a product-specific redistribution record. Free registration is not a rejection by itself. Reference-only records are allowed for reference use, reported as not comparable for analytical comparison, and rejected for analytical processing or public export. A later domain pack may retain ordered source-provenance records without merging source identities. `DATA_LICENSES.md` defines the licence boundary, while every public OSM-derived pack carries a checked ODbL notice file and manifest record.
