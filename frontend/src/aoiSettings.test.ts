...
describe("AOI settings request state", () => {
    ...
    expect(
      buildRuntimeRequest(
        "administrative_selection",
        {
          ...
          latitude: "",
          radius: "",
          unitIds: ["county_rybnicki", "county_rybnik_city", "county_rybnicki"],
        },
        ["water", "power", "water"],
      ),
    ).toEqual({
  ...
  it("finds a single province root for county/gmina selections", () => {
    ...
    expect(administrativeSelectionRoots(["county_2473", "gmina_2473011"], [...units])).toEqual([
      "voivodeship_24",
    ]);
    expect(administrativeSelectionRoots(["gmina_2473011", "voivodeship_14"], [...units])).toEqual([
      "voivodeship_14",
      "voivodeship_24",
    ]);
  ...
  it("validates administrative selection rules (no voivodeship, max 3 counties, gminas across up to 3 counties)", () => {
    ...
    expect(validateAdministrativeUnitSelection(["county_2473", "county_1465"], units).valid).toBe(
      false,
    );

    // 3. Up to 3 counties allowed
    expect(
      validateAdministrativeUnitSelection(["county_2473", "county_2412", "county_2415"], units)
        .valid,
    ).toBe(true);
    ...
    expect(
      validateAdministrativeUnitSelection(
        ["county_2473", "county_2412", "county_2415", "county_2402"],
        units,
      ).valid,
    ).toBe(false);
    ...
    expect(
      validateAdministrativeUnitSelection(
        ["gmina_2473011", "gmina_2412012", "gmina_2415012"],
        units,
      ).valid,
    ).toBe(true);
    ...
    expect(
      validateAdministrativeUnitSelection(
        ["gmina_2473011", "gmina_2412012", "gmina_2415012", "gmina_2402012"],
        units,
      ).valid,
    ).toBe(false);
  });
  ...
  it("builds a point/radius request and enforces max 30km radius for custom points", () => {
    expect(
      buildRuntimeRequest(
        "point_radius",
        { longitude: "18.5", latitude: "50.1", radius: "1000", unitIds: [] },
        ["power"],
      ),
    ).toEqual({
      aoi: { type: "point_radius", longitude: 18.5, latitude: 50.1, radius_m: 1000 },
      profiles: ["power"],
    });
    expect(
      buildRuntimeRequest(
        "point_radius",
        { longitude: " 18,546 ", latitude: " 50,102 ", radius: " 20000 ", unitIds: [] },
        ["power"],
      ),
    ).toEqual({
      aoi: { type: "point_radius", longitude: 18.546, latitude: 50.102, radius_m: 20000 },
      profiles: ["power"],
    });
    expect(() =>
      buildRuntimeRequest(
  ...
  it("explains that a failed acquisition preserves the existing map", async () => {
    const response = new Response(
      JSON.stringify({ error: "worker_failed", message: "worker_failed: Overpass timed out." }),
      { status: 502 },
    );
    await expect(runtimeRequestError(response)).resolves.toBe(
      "No new AOI snapshot was published; the existing map was left unchanged. worker_failed: Overpass timed out.",
    );
  });
  ...
  it("uses a clear fallback when an API error has no structured message", async () => {
    await expect(
      providerResponseMessage(
        ...
        "Administrative catalogue could not be read (HTTP 502).",
      ),
    ).resolves.toBe("Administrative catalogue could not be read (HTTP 502).");
  });
  ...
  it("provides a default 35km outline for Rybnik snapshot", () => {
    ...
    expect(DEFAULT_AOI_OUTLINE?.coordinates[0]?.[0]).toEqual(
      DEFAULT_AOI_OUTLINE?.coordinates[0]?.at(-1),
    );
  });
