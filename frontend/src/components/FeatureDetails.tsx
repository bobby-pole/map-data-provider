...
export function FeatureDetails({ aoiId, selected, onDetailChange }: Props) {
  ...
  useEffect(() => {
    ...
    void fetch(base)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(`Feature details: HTTP ${response.status}`);
        }
  ...
  if (!selected) {
    return (
      <section className="inspectorSection">
        ...
        <p className="muted">Click a visible map feature to inspect its source-backed popup.</p>
      </section>
    );
  }
  ...
  const inspection = featureInspection(
    activeDetail ? { ...selected, feature: activeDetail.feature } : selected,
  );
  return (
    <section className="inspectorSection featureDetails">
      ...
      <p className="muted">
        Source attributes and external OSM links are in the map popup. Power-line selection is
        available only in that popup, so there is one consistent interaction path.
      </p>
      <dl>
        <dt>source</dt>
        <dd>{inspection.source}</dd>
        <dt>readiness</dt>
        <dd>{inspection.readiness}</dd>
      </dl>
      {selected.feature.geometry?.type === "LineString" && selected.layer.domain !== "power" && (
        <section className="selectedLineGeometry">
          ...
          <dl>
            <dt>start</dt>
            <dd>{selected.feature.geometry.coordinates[0].map((c) => c.toFixed(5)).join(", ")}</dd>
            <dt>end</dt>
            <dd>
              {((selected.feature.geometry.coordinates as number[][]).at(-1) ?? [])
                .map((c) => c.toFixed(5))
                .join(", ")}
            </dd>
            {typeof selected.feature.properties.road_class === "string" && (
              <>
                <dt>road class</dt>
                <dd>{selected.feature.properties.road_class}</dd>
              </>
            )}
          </dl>
          <p className="muted">
            Highlighted on map. Geometry endpoints reflect original source coordinates; no network
            routing or connectivity is inferred.
          </p>
        </section>
      )}
