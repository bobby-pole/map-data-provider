...
describe("feature inspection", () => {
  it("keeps rendered provider evidence, attribution and compact MVT attributes together", () => {
    ...
    if (!layer) {
      throw new Error("Expected a preview layer.");
    }
    ...
    expect(inspection.providerAttributes).toEqual(
      expect.arrayContaining([
        { name: "source_id", value: "way/1" },
        { name: "asset_type", value: "water main" },
      ]),
    );
    expect(inspection.providerAttributes).not.toEqual(
      expect.arrayContaining([{ name: "osm_tags", value: '{"man_made":"pipeline"}' }]),
    );
  });
  ...
  it("inspects transport road features and preserves provider-normalized road_class", () => {
    const transportPresentation: MapPresentation = {
      ...
      attribution: "© OpenStreetMap contributors",
      archive_url: "/api/aoi/fixture_aoi/presentations/transport/archive",
      layers: [
        {
          artifact_id: "transport.roads",
          source_layer: "transport.roads",
          feature_count: 1,
          source: "OpenStreetMap",
          confidence: "medium",
          readiness: "usable_with_limitations",
          limitations: ["Transport limitation."],
          attribution: "© OpenStreetMap contributors",
          source_provenance: [{ source_id: "openstreetmap", contribution_role: "primary" }],
        },
      ],
    ...
    if (!layer) {
      throw new Error("Expected transport layer.");
    }
    const inspection = featureInspection({
      layer,
      feature: {
        ...
        properties: {
          ...
          source: "OpenStreetMap",
          source_id: "way/10",
          osm_tags: { highway: "primary", ref: "DK78" },
        },
        geometry: {
          type: "LineString",
          coordinates: [
