...
const symbolDefinitions: Record<MapSymbolKind, { color: string; path: string }> = {
  ...
  "fire-station": {
    color: "#fb923c",
    path: "M12 3c2 4 5 5 5 10a5 5 0 1 1-10 0c0-3 2-5 3-7 0 3 2 3 2-3Zm0 9c-1 1-2 2-2 3a2 2 0 0 0 4 0c0-1-1-2-2-3Z",
  },
  ...
  "rail-station": {
    color: "#8b5cf6",
    path: "M7 4h10v12a3 3 0 0 1-3 3H10a3 3 0 0 1-3-3V4Zm3 3h4M9 11h1m4 0h1M9 19l-2 2m8-2 2 2",
  },
  ...
  "water-facility": {
    color: "#06b6d4",
    path: "M12 3c4 5 6 8 6 11a6 6 0 0 1-12 0c0-3 2-6 6-11Zm-3 12c1 2 5 2 6-1",
  },
  "gas-facility": { color: "#fb923c", path: "M8 5h8v14H8Zm2 3h4m-4 4h4m-4 4h4M6 5v14m12-14v14" },
  "sewer-facility": {
    color: "#a16207",
    path: "M5 12a7 7 0 1 0 14 0 7 7 0 1 0-14 0Zm3-2h8m-8 4h8m-4-6v8",
  },
  "telecom-tower": {
    color: "#a78bfa",
    path: "M12 4v16M8 20h8M9 8a5 5 0 0 0 0 8m6-8a5 5 0 0 1 0 8M6 5a9 9 0 0 0 0 14m12-14a9 9 0 0 1 0 14",
  },
  "telecom-facility": { color: "#8b5cf6", path: "M5 5h14v14H5Zm3 3h8m-8 4h8m-8 4h5" },
  "heat-facility": {
    color: "#ef4444",
    path: "M12 3c3 3 5 5 5 8 0 4-2 6-5 10-3-4-5-6-5-10 0-3 2-5 5-8Zm-2 8c1 1 3 1 4 0m-5 4c2 1 4 1 6 0",
  },
  industrial: { color: "#94a3b8", path: "M4 20V9l6 3V8l6 3V5l4 2v13H4Zm4-3h2m3 0h2m3 0h1" },
  military: {
    color: "#65a30d",
    path: "M12 3 4 7v6c0 5 3.5 8 8 10 4.5-2 8-5 8-10V7l-8-4Zm0 4 1.5 3.5 3.5.5-2.5 2.5.5 3.5-3-1.8-3 1.8.5-3.5L7 11l3.5-.5L12 7Z",
  },
  generic: { color: "#38bdf8", path: "M12 5a7 7 0 1 1 0 14 7 7 0 0 1 0-14Zm0 4v6m-3-3h6" },
};

export function mapSymbolKinds(): readonly MapSymbolKind[] {
  return iconKinds;
}
export function mapSymbolImageId(kind: MapSymbolKind): string {
  return `mdq-symbol:${kind}`;
}
...
export function pointSymbolKind(layer: PreviewLayer): MapSymbolKind {
  ...
  if (artifact === "power.assets") {
    return "power-substation";
  }
  if (artifact === "power.supports") {
    return "power-tower";
  }
  if (artifact.includes("hospital")) {
    return "hospital";
  }
  if (artifact.includes("fire")) {
    return "fire-station";
  }
  if (artifact.includes("police")) {
    return "police";
  }
  if (artifact.includes("ambulance")) {
    return "ambulance";
  }
  if (artifact.includes("administration")) {
    return "townhall";
  }
  if (artifact.includes("education")) {
    return "school";
  }
  if (artifact.includes("post")) {
    return "post-office";
  }
  if (artifact.includes("community")) {
    return "library";
  }
  if (artifact.includes("stations")) {
    return "rail-station";
  }
  if (artifact.includes("aviation")) {
    return "aviation";
  }
  if (artifact.includes("crossings")) {
    return "crossing";
  }
  if (artifact.startsWith("water.")) {
    return "water-facility";
  }
  if (artifact.startsWith("gas.")) {
    return "gas-facility";
  }
  if (artifact.startsWith("sewer.")) {
    return "sewer-facility";
  }
  if (artifact === "telecom.towers") {
    return "telecom-tower";
  }
  if (artifact.startsWith("telecom.")) {
    return "telecom-facility";
  }
  if (artifact.startsWith("district_heating.")) {
    return "heat-facility";
  }
  if (artifact.includes("military")) {
    return "military";
  }
  if (artifact.startsWith("industrial.")) {
    return "industrial";
  }
  return "generic";
}
...
export function pointSymbolExpression(layer: PreviewLayer): string | ExpressionSpecification {
  if (layer.artifact.artifact_id === "power.assets") {
    return [
      "match",
      ["get", "asset_type"],
      "substation",
      mapSymbolImageId("power-substation"),
      "transformer",
      mapSymbolImageId("power-transformer"),
      "plant",
      mapSymbolImageId("power-plant"),
      "generator",
      mapSymbolImageId("power-generator"),
      "switch",
      mapSymbolImageId("power-switchgear"),
      "terminal",
      mapSymbolImageId("power-switchgear"),
      "converter",
      mapSymbolImageId("power-switchgear"),
      ...
      "portal",
      mapSymbolImageId("power-portal"),
      "pole",
      mapSymbolImageId("power-pole"),
      "utility_pole",
      mapSymbolImageId("power-pole"),
      mapSymbolImageId("generic"),
    ] as ExpressionSpecification;
  }
  if (layer.artifact.artifact_id !== "power.supports") {
    return mapSymbolImageId(pointSymbolKind(layer));
  }
  return [
    "match",
    ["get", "asset_type"],
...
export function legendItemsForLayer(layer: PreviewLayer): SymbolLegendItem[] {
  if (layer.artifact.artifact_id === "power.assets") {
    return [
      { kind: "power-substation", label: "Substation" },
      { kind: "power-transformer", label: "Transformer" },
      { kind: "power-plant", label: "Power plant" },
      { kind: "power-generator", label: "Generator" },
      { kind: "power-switchgear", label: "Switchgear" },
      { kind: "power-tower", label: "Transmission tower" },
      { kind: "power-portal", label: "Line portal" },
      { kind: "power-pole", label: "Utility pole" },
    ];
  }
  if (layer.artifact.artifact_id === "power.supports") {
    return [
  ...
  return [
    {
      kind: pointSymbolKind(layer),
      label: layer.artifact.artifact_id
        .replace(/[._]/g, " ")
